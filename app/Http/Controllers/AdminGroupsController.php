<?php namespace App\Http\Controllers;
	...
	class AdminGroupsController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col = [];
			$this->col[] = ["label"=>"Guide Id","name"=>"guide_id","join"=>"guides,id"];
			...
			$this->form = [];
			$this->form[] = ['label'=>'Guide Id','name'=>'guide_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'guides,id'];
			$this->form[] = ['label'=>'Route Id','name'=>'route_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'routes,id'];
			$this->form[] = ['label'=>'Start Plan','name'=>'start_plan','type'=>'date','validation'=>'required|date','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Start Fact','name'=>'start_fact','type'=>'date','validation'=>'required|date','width'=>'col-sm-10'];
	    ...
	    */
		...
		public function getIndex() {
			...
			$this->cbView('admin-modules/groups/groups_index_view', $data);
		}
		...
		public function getAdd() {
			...
			$data = [];
			$data['page_title'] = 'Добавление новой группы';
			$data['routes'] = Route::get();
			$data['guides'] = Cms_user::where('id_cms_privileges','2')->get();
			
			$this->cbView('admin-modules/groups/groups_add_view',$data);
		}
		...
		public function getEdit($id) {
			...
			$data = [];
			$data['page_title'] = 'Редактирование группы';
			$data['row'] = Group::where('id', $id)->first();
			$data['routes'] = Route::get();
			$data['guides'] = Cms_user::where('id_cms_privileges','2')->get();			
